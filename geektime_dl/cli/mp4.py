...
class Mp4(Command):
    ...
    def run(self, cfg: dict):
        ...
        for course_id in course_ids:
            ...
            print(colored('开始下载视频:{}-{}'.format(
                course_id, course_data['title']), 'green'))
            pbar_desc = '数据爬取中:{}'.format(course_data['title'][:10])
            data = dc.get_course_content(course_id, pbar_desc=pbar_desc, force=True)
    ...
    def _parse_url(post_content: dict, hd_only: bool):
        video = (post_content.get('video') or {}).get("hls_medias", [])
        if hd_only:  # some post has sd mp4 only
            url = (video[2].get('url') or video[1].get('url') or video[0].get('url'))
        else:
            url = video[0].get('url')
